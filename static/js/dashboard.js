document.addEventListener('DOMContentLoaded', async () => {
  ...
  let userLogado = { id: null, is_admin: false };
  ...
  async function loadAnuncios() {
    try {
      const url = anuncioId ? `${baseUrl}/anuncios/${anuncioId}` : `${baseUrl}/anuncios`;
    const method = anuncioId ? 'PUT' : 'POST';
    const res = await fetch(url, { method, headers: jsonHeaders, headers: jsonHeaders });
  ...
  function renderAnuncios(anuncios) {
    ...
    anuncios.forEach(a => {
      ...
      card.innerHTML = `
        ...
        ${(userLogado.is_admin || a.usuario_id === userLogado.id)
          ? `
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const anuncio = anuncios.find(a => a.id == btn.dataset.id);
  ...
  async function createAnuncio(e) {
    ...
    const anuncioId = petForm.elements['anuncioId'].value;
    const payload = {
      titulo:     nome,
      idade:      idade,
      sexo:       sexo,
    ...
    try {
      const url = anuncioId ? `${baseUrl}/anuncios/${anuncioId}` : `${baseUrl}/anuncios`;
    const method = anuncioId ? 'PUT' : 'POST';
    const res = await fetch(url, { method, headers: jsonHeaders,
        method:  'POST',
        headers: jsonHeaders,
  ...
  function abrirEdicao(anuncio) {
    modal.style.display = 'block';
    petForm.elements['petName'].value = anuncio.titulo;
    petForm.elements['petAge'].value = anuncio.idade;
    petForm.elements['petPhone'].value = anuncio.telefone.replace('+55','');
    petForm.elements['petSex'].value = anuncio.sexo;
    petForm.elements['existingImageUrl'].value = anuncio.imagem;
    petForm.elements['anuncioId'].value = anuncio.id;
  }
