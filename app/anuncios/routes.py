...
@anuncios_bp.route('/anuncios/<int:id>', methods=['GET'])
@token_required
def get_anuncio(current_user, id):
    anuncio = Anuncio.query.get_or_404(id)
    data = anuncio.to_dict()
    if data.get('imagem'):
        data['imagem'] = url_for('static', filename=f'uploads/{data["imagem"]}', _external=True)
